...
@section('content')
    <div class="mb-4 d-flex align-items-center justify-content-between">
        ...
        <div class="gap-2 d-flex">
            <form class="gap-2 d-flex" method="GET" action="{{ route('backoffice.employees.index') }}">
    ...
    @if (session('import_errors'))
        <div class="alert alert-warning">
            <div class="mb-2 fw-semibold">Erreurs d'import</div>
            <ul class="mb-0">
                    ...
                    <tbody>
                        @foreach ($employees as $employee)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="bo-pill {{ ($employee->medical_visits_count ?? 0) > 0 ? 'bg-success text-white' : 'bg-light text-dark border' }}">
                                        {{ ($employee->medical_visits_count ?? 0) > 0 ? 'Effectuée' : 'Non' }}
                                    </span>
                                </td>
                                <td>
                                    <span
                                        class="bo-pill {{ ($employee->qhse_visits_count ?? 0) > 0 ? 'bg-success text-white' : 'bg-light text-dark border' }}">
                                ...
                                @if (auth()->user()->role === 'admin')
                                    <td class="gap-2 text-end d-flex justify-content-end">
                                        <a class="btn btn-sm btn-outline-dark"
                                            href="{{ route('backoffice.employees.edit', $employee) }}">Modifier</a>
                                        <form method="POST"
                                            action="{{ route('backoffice.employees.destroy', $employee) }}"
                                            class="d-inline">
                                            @csrf
                                            @method('DELETE')
